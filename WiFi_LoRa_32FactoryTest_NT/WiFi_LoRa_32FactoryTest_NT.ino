...
void WIFISetUp(void)
{
	...
	WiFi.begin("VM3113128_2_4","4hxpfmwFnnhh");
	delay(1000);
	...
	while(WiFi.status() != WL_CONNECTED && count < 10)
	{
		count ++;
		delay(2000);
		Heltec.display -> drawString(0, 0, "Connecting to preferred Wifi...");
		Heltec.display -> display();
	}
	...
	{
		Heltec.display -> drawString(0, 0, "Wifi Connected...OK.");
		Heltec.display -> display();
		delay(500);
	}
	else
	{
		Heltec.display -> clear();
		Heltec.display -> drawString(0, 0, "Wifi Connecting...Failed");
		Heltec.display -> display();
		//w                       hile(1);
	}
...
void WIFIScan(unsigned int value)
{
	...
	for(i=0;i<value;i++)
	{
		Heltec.display -> drawString(0, 20, "Wifi scan start...");
		Heltec.display -> display();

		int n = WiFi.scanNetworks();
		Heltec.display -> drawString(0, 30, "Wifi scan done");
		Heltec.display -> display();
		delay(2000);
		Heltec.display -> clear();
		...
		{
			Heltec.display -> clear();
			Heltec.display -> drawString(0, 0, "no Wifi network found");
			Heltec.display -> display();
			while(1);
		}
		else
...
void loop()
{
	if(millis() - lastSendTime > interval)//waiting LoRa interrupt
	{
		...
		Heltec.display -> drawString(0, 50, "LoRa Packet " + (String)(counter-1) + " sent done");
		Heltec.display -> display();
